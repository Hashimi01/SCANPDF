...
def detect_language_from_pdf_name(pdf_name: str) -> str:
    ...
    القواعد:
    - إذا كان رقم الملف "1" أو "-1" (بدون شرطة أو بشرطة واحدة) = عربي (ara)
    - إذا كان رقم الملف "--1" (بشرطتين) = فرنسي (fra)
    
    Args:
        pdf_name: اسم ملف PDF (مثلاً: "-1463.pdf" أو "--1463.pdf")
    ...
    # التحقق من عدد الشرطات في البداية
    if name_without_ext.startswith("--"):
        # شرطتان = فرنسي
        return "fra"
    else:
        # بدون شرطة أو بشرطة واحدة = عربي
        return "ara"
