...
def detect_language_from_pdf_name(pdf_name: str) -> str:
    ...
    - خلاف ذلك = عربي (ara) - سواء كان أمامه "-" واحدة أم لا شيء
    
    أمثلة:
    - "--1463.pdf" = فرنسي (يبدأ بـ "--")
    - "778--.pdf" = فرنسي (يحتوي على "--" في المنتصف)
    - "1001--.pdf" = فرنسي (يحتوي على "--" في المنتصف)
    - "-1463.pdf" = عربي (شرطة واحدة فقط)
    - "1463.pdf" = عربي (لا يوجد شرطة)
    - "book.pdf" = عربي (لا يوجد شرطة)
    
    ملاحظة مهمة:
    - كل الملفات التي لا تحتوي على "--" هي عربية (سواء كانت أمامها "-" واحدة أم لا)
    - كل الملفات التي تحتوي على "--" هي فرنسية
    ...
    # لا نحتاج إزالة الامتداد - يمكن البحث مباشرة
    if "--" in pdf_name:
        # يحتوي على "--" = فرنسي
        return "fra"
    else:
        # لا يحتوي على "--" = عربي (سواء كان "-" واحدة أم لا شيء)
        return "ara"
